...
(function($){
    ...
    MultiSelect.prototype = {
        ...
        load: function() {
            ...
            $(instance.element).after('<div id="ms-list-'+ instance.listNumber +'" class="ms-options-wrap fisco-option-padding"><button type="button"><span>None Selected</span></button><div class="ms-options"><ul></ul></div></div>');
			/*kimth 테스트 클래서 추가*/
            ...
            placeholder.on( 'mousedown', function( event ){
                ...
                if( optionsWrap.closest('.ms-options-wrap').hasClass('ms-active') ) {
					...
					/* fisco kimth DIV내에 스크롤이 존재하는곳에 멀티 콤보를 그리기 위한 위치지정 추가 Start*/
					var targetLeft = 0;
					var targetWidth = 0;
					
					/*
						선택한곳이 [button] 혹은 [span] 영역일 수 있음 어느 것이 선택 되었느냐에 따르 target의 offsetLeft값이 달라짐
						실제 필요한 것은 [button]의 offsetLeft값임으로 [span]일 경우 상위항목인 [button]의 offsetLeft값을 가져와서 계산
					*/
					if($(event.target)[0].localName == "span"){
						targetLeft = $(event.target)[0].parentElement.offsetLeft;
						targetWidth = $(event.target)[0].parentElement.offsetWidth;
					}else{
						targetLeft = $(event.target)[0].offsetLeft;
						targetWidth = $(event.target)[0].offsetWidth;
					}
					var objLeft = targetLeft - $("#btn-box-search")[0].scrollLeft;
					optionsWrap.css('left', objLeft);
					optionsWrap.css('width', targetWidth);
					/* fisco kimth DIV내에 스크롤이 존재하는곳에 멀티 콤보를 그리기 위한 위치지정 추가 End*/
                }
